...
@csrf_exempt  # Only for testing! Remove in production and implement proper CSRF protection
def mark_attendance(request):
    ...
    try:
        ...
        student_roll_no = data.get('UID')  # Using UID as roll_no
        subject_code = data.get('subject_code', 'CS101')  # Default to CS101 for MVP
        is_present = data.get('ispresent', True)
        
        if not student_roll_no:  # Only check for UID/roll_no
            return JsonResponse({'error': 'Student UID is required'}, status=400)
...
@login_required
def add(request):
    return render(request, "edu/final_index.html")