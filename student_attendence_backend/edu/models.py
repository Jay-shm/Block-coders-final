...
class Attendance(models.Model):
    ...
    @classmethod
    def mark_attendance(cls, student_roll_no, subject_code, is_present=True):
        try:
            student = StudentProfile.objects.get(roll_no=student_roll_no)
            subject = Subject.objects.get(code=subject_code)
            
            # Simplified: Create or update today's attendance
            attendance, created = cls.objects.get_or_create(
                ...
                defaults={
                    ...
                    'hours_attended': 1 if is_present else 0  # Simplified to 1 hour per session
                }
            )
            
            if not created:
                attendance.is_present = is_present
                attendance.hours_attended = 1 if is_present else 0
                attendance.save()
                
            return attendance
            
        except (StudentProfile.DoesNotExist, Subject.DoesNotExist) as e:
            raise ValueError(f"Invalid student roll number or subject code: {str(e)}")
